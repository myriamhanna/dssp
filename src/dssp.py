""" This program is the non-exhaustive implementation of the DSSP algorithm, it assigns basic secondary structure: Helices and beta-sheets """
...
class Pdb:
    ...
    def read_pdb(self, filename):
        """method that open the file and reads the line"""
        with open(filename) as filin:
            return filin.readlines()

    #    def add_H(self):
    #unfinished method that is supposed to add H when the PDB file is missing hydrogen atoms

    #        for line in self.lines:
    #            if line[12:16] not in self.atoms
    ...
    def energy_Hbonds(self, idx1, idx2):
        ...
        list_resid = list(self.content.keys())

        r_ON = math.sqrt((self.content[list_resid[idx1]][9] - self.content[list_resid[idx2]][1])**2 +
                       (self.content[list_resid[idx1]][10] - self.content[list_resid[idx2]][2])**2 +
                       (self.content[list_resid[idx1]][11] - self.content[list_resid[idx2]][3])**2)
        r_CH = math.sqrt((self.content[list_resid[idx1]][5] - self.content[list_resid[idx2]][13])**2 +
                       (self.content[list_resid[idx1]][6] - self.content[list_resid[idx2]][14])**2 +
                       (self.content[list_resid[idx1]][7] - self.content[list_resid[idx2]][15])**2)
        r_OH = math.sqrt((self.content[list_resid[idx1]][9] - self.content[list_resid[idx2]][13])**2 +
                       (self.content[list_resid[idx1]][10] - self.content[list_resid[idx2]][14])**2 +
                       (self.content[list_resid[idx1]][11] - self.content[list_resid[idx2]][15])**2)
        r_CN = math.sqrt((self.content[list_resid[idx1]][5] - self.content[list_resid[idx2]][1])**2 +
                       (self.content[list_resid[idx1]][6] - self.content[list_resid[idx2]][2])**2 +
                       (self.content[list_resid[idx1]][7] - self.content[list_resid[idx2]][3])**2)
    ...
    def find_nturns(self):
        ...
        print("RESIDUES IN N-TURNS")
        print("-------------------")
        print("\n")
        print("3-turn", self.count(self.dict_nturns, 3))
        print("4-turn", self.count(self.dict_nturns, 4))
        print("5-turn", self.count(self.dict_nturns, 5), "\n")
    ...
    def find_bridges(self):
        ...
        print("RESIDUES IN BRIDGES")
        print("-------------------")
        print("\n")
        print("ANTIPARALLEL", self.count(self.dict_bridges, 'AP'))
        print("PARALLEL", self.count(self.dict_bridges, 'P'), "\n")
    ...
    def merge_tuples(self,obj):
        ...
        for i in range(0, len(obj)):
            merged_list.append(obj[i][0])
            merged_list.append(obj[i][1])
        return merged_list

    def find_sheets(self):
        """ method that finds sheets using the ladders previously found"""
    ...
    def find_helices(self):
        """method that finds helices based on n-turns """
